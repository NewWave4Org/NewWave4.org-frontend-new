  ...
  return (
    <section className="our-mission lg:py-14 py-7 lg:mb-20 mb-10 bg-skyBlue-300">
      <div className="container mx-auto px-4">
        <h3 className="section-title lg:text-[34px] text-[48px] text-center text-font-accent mb-10 font-bold lora-family uppercase">
        ...
        <div className="flex flex-col items-baseline gap-6 md:flex-row md:justify-between">
          {ourMission?.map((item, index) => {
            const ourMissionDescriptionText = convertDraftToHTML(
            ...
            return (
              <div
                key={item.id}
                className="flex w-full flex-col items-center text-center text-font-primary md:w-1/3 lg:px-12 md:px-6 md:py-0 sm:w-full sm:px-0 sm:py-4"
              >
                <span className="relative h-[124px] block">
                  <Image
                    src={icon}
                    alt={item.translatable_text_title ?? 'our-mission'}
                    width={iconWidth}
                    height={124}
                    className="mb-4"
                  />
                </span>
                <h4 className="mb-3 text-h4 lora-family lg:text-[24px] text-[29px] font-bold">
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={{
                    __html: ourMissionDescriptionText,
                  }}
                />
              </div>
            );

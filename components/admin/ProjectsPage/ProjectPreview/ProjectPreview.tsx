...
function ProjectPreview({
  ...
  project: GetArticleByIdResponseDTO | undefined;
}) {
  const quoteText = project?.contentBlocks?.find(
    item => item.contentBlockType === 'QUOTE',
  );

  const siteLink = project?.contentBlocks?.find(
    item => item.contentBlockType === 'LINK_TO_SITE',
  )?.siteUrl;

  const typeSocialMedia = project?.contentBlocks?.find(
    item => item.contentBlockType === 'SOCIAL_MEDIA',
  )?.typeSocialMedia;
  const linkSocialMedia = project?.contentBlocks?.find(
    item => item.contentBlockType === 'SOCIAL_MEDIA',
  )?.socialMediaUrl;
  ...
  const projectSections = project?.contentBlocks?.filter(
    item => item.contentBlockType === 'SECTION',
  );

  const projectVideoUrl = project?.contentBlocks?.find(

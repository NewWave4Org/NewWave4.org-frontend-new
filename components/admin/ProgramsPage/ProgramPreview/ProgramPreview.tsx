...
function ProgramPreview({
  ...
  program: GetArticleByIdResponseDTO | undefined;
}) {
  const titleProgram = program?.title;
  const pageBanner = program?.contentBlocks?.find(
    item => item.contentBlockType === 'PAGE_BANNER',
  )?.files;
  const dateProgram = program?.contentBlocks?.find(
    item => item.contentBlockType === 'DATE_PROGRAM',
  )?.date;
  const descriptionProgram = program?.contentBlocks?.find(
    item => item.contentBlockType === 'DESCRIPTION_PROGRAM',
  )?.editorState;

  const blocksWithPhoto = program?.contentBlocks?.filter(
    item => item.contentBlockType === 'SECTION_WITH_PHOTO',
  );
  const filteredBlocksWithPhoto = blocksWithPhoto?.filter(item => {
    return (
      item.sectionTitle !== '' || item.text !== '' || item.files.length > 0
    );
  });

  const blocksWithText = program?.contentBlocks?.filter(

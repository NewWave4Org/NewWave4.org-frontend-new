...
function AboutPageClientSide() {
  ...
  const ourMission =
    aboutPage?.contentBlocksToShow?.filter(
      item => item.contentBlockType === 'MISSION_BLOCK',
    ) || [];
  const quote = aboutPage?.contentBlocksToShow?.find(
    item => item.contentBlockType === 'QUOTE',
  );
  const ourHistoryTitle = aboutPage?.contentBlocksToShow?.find(
    item => item.contentBlockType === 'OUR_HISTORY_TITLE',
  )?.translatable_text_title;
  const ourHistoryDescription = aboutPage?.contentBlocksToShow?.find(
    item => item.contentBlockType === 'OUR_HISTORY_DESCRIPTION',
  );
  const ourHistoryPhotos = aboutPage?.contentBlocksToShow?.find(
    item => item.contentBlockType === 'PHOTOS',
  );
  const ourTimeLine =
    aboutPage?.contentBlocksToShow?.filter(
      item => item.contentBlockType === 'HISTORY_OF_FORMATION',
    ) || [];
  ...
  return (
    <div>
      <Hero
        ...
        pageBanner="/about/about-us__banner.png"
      />
      ...
      <HistoryCard
        ourHistoryTitle={ourHistoryTitle}
        ourHistoryDescription={ourHistoryDescription}
        ourHistoryPhotos={ourHistoryPhotos}
      />

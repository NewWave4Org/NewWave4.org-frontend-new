...
const GeneralSlider: React.FC<GeneralSliderProps> = ({
  ...
  const [emblaRef, emblaApi] = useEmblaCarousel(
    { loop: loop, align: slideCount == 2 ? 'start' : 'center' },
  ...
  return (
    <div className="relative embla h-[544px] group/arrows">
        ...
        <div className={`${initSlider ? 'flex' : 'block'}`}>
          {slides?.map((slide, index) => {
                ...
                {slideHover && (
                  ...
                  >
                    <div className="max-w-[545px] pb-[100px] pl-[55px] flex flex-col w-full h-full justify-end">
                      <div className="text-white text-h4 mb-4 font-ebGaramond">
                        {slide.title}
                      </div>
            ...
            return (
              <div
                key={index}
                className={`relative embla-slide group flex-shrink-0 h-[544px] group/slide aspect-[824/544] ${slideWidthClass}`}
                style={{ '--slide-size': slideSize } as React.CSSProperties}
              >
                {typeof slide.link === 'string' && slide.link.trim() !== '' ? (
                  <Link href={slide.link}>{slideContent}</Link>
                ) : (
                  slideContent
                )}
              </div>
      ...
      {showArrows && initSlider && (
        <>
          <button
            className="embla-slide-btn left-4 group-hover/arrows:opacity-100 transition-all duration-300"
            onClick={scrollPrev}
          >
            <ArrowLeft4Icon size="32" color="#fafafa" />
          </button>
          <button
            className="embla-slide-btn right-4 flex group-hover/arrows:opacity-100 transition-all duration-300 justify-center items-center"
            onClick={scrollNext}
          >

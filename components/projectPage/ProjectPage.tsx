...
type IProjectProps = IArticleBody & {
  titleToShow?: string;
  contentBlockToShow?: any[];
};
...
function ProjectPage({
  ...
  const videoLink = project?.contentBlockToShow?.find(
    item => item.contentBlockType === 'VIDEO',
  )?.videoUrl;
  const embedUrl = convertYoutubeUrlToEmbed(videoLink);

  const quoteText = project?.contentBlockToShow?.find(
    item => item.contentBlockType === 'QUOTE',
  );
  const siteLink = project?.contentBlockToShow?.find(
    item => item.contentBlockType === 'LINK_TO_SITE',
  )?.siteUrl;

  const typeSocialMedia = project?.contentBlockToShow?.find(
    item => item.contentBlockType === 'SOCIAL_MEDIA',
  )?.typeSocialMedia;
  const linkSocialMedia = project?.contentBlockToShow?.find(
    item => item.contentBlockType === 'SOCIAL_MEDIA',
  )?.socialMediaUrl;
  const nameSocialMedia = typeSocialMediaList.find(
    item => item.value === typeSocialMedia,
  )?.label;

  const projectSections = project?.contentBlockToShow?.filter(
    item => item.contentBlockType === 'SECTION',
  );
  ...
  return (
    <div className="project-section" id={`project-${project.id}`}>
      ...
      {quoteText?.translatable_text_editorState !== null &&
        quoteText?.translatable_text_editorState?.blocks[0].text !== '' && (
      ...
      <NewsEvents
        textLink="projects_page.all_project_new"
        link="/news"
        titleEvents="projects_page.news_events_project"

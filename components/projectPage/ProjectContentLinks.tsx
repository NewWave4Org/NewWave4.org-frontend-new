...
function ProjectContentLinks({
  index,
  siteLink,
  linkSocialMedia,
  nameSocialMedia,
  showLinksInIndex,
}: IProjectContentLinks) {
  const [isHovered, setIsHovered] = useState(false);
  const t = useTranslations();
  return (
    <>
      <div className="mt-6 text-font-primary text-body">
        {t('projects_page.learn_more')}
      </div>
        ...
        <div className="flex gap-x-4 mt-4">
          {siteLink && (
            ...
            >
              <span className="mr-1 text-medium1 inline-block mt-[-2px]">
                {t('projects_page.on_the_site')}
              </span>
              <div className="mt-[3px]">
                <ArrowRightIcon
                  size="20"
                  className="hover:duration-500 duration-500"
                  color={isHovered ? 'white' : '#3D5EA7'}
                />
              </div>
            </LinkBtn>
          )}
          ...
          {linkSocialMedia && (
            <LinkBtn href={linkSocialMedia} className="px-[26px]">
              <span className="inline-block mr-1 text-medium1">
                {t('projects_page.on_the_page')} {nameSocialMedia}
              </span>

...
import articleContentReducer, {
  removeArticle,
  removeArticleFromArchive,
} from './article-content_slice';
...
describe('article-content_slice', () => {
  ...
  describe('getAllArticle.fulfilled', () => {
    it('stores items/totalPages and marks the requested type as loaded', () => {
      ...
      const state = articleContentReducer(
        initialState(),
        getAllArticle.fulfilled(payload as never, 'req-id', {
          articleType: ArticleTypeEnum.PROJECT,
        } as never),
      );
  ...
  describe('removeArticle', () => {
    it('removes only the matching id from the given articleType', () => {
      ...
      const state = articleContentReducer(
        seeded,
        removeArticle({ id: 1, articleType: ArticleTypeEnum.NEWS }),
      );

...
3. **Production configuration** — three pieces, all outside the workflow files (see [ADR 0007](./decisions/0007-separate-production-image.md)):
   - **`production` GitHub Environment** — created 2026-09-20 with a required reviewer, so the manual `deploy-to-kubernetes.yml` dispatch to `namespace: production` pauses for approval. It must carry its own **`KUBECONFIG`** and **`VALUES_YAML`** environment secrets (production ingress hosts `www.newwave4.org` / `newwave4.org`, TLS secret, replicas). If `VALUES_YAML` is missing there, GitHub silently falls back to the repo-level (staging) one — the deploy's "Guard production values" step exists to catch exactly that and fails the run.
   - **Repo-level `PROD_NEXT_PUBLIC_*` secrets** — `PROD_NEXT_PUBLIC_NEWWAVE_API_URL`, `PROD_NEXT_PUBLIC_PAYPAL_CLIENT_ID`, `PROD_NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEYS` (required), `PROD_NEXT_PUBLIC_SITE_URL` (set it — otherwise canonical URLs and `sitemap.xml` in the production image point at staging), `PROD_NEXT_PUBLIC_STRIPE_WEBHOOK_URL` (ceremonial). Until the three required ones exist, `release.yml`'s `docker-publish-production` job skips with a warning and no `newwave4-frontend-production:X.Y.Z` image is published, so `namespace: production` deploys fail at image pull.
   - **A production backend** for `PROD_NEXT_PUBLIC_NEWWAVE_API_URL` to point at. `backend-prod` (`blog.api.newwave4.org`) still runs the previous API and does not serve this frontend's routes; `www.newwave4.org` is served by the previous frontend from the `frontend-prod` namespace. Going live is therefore an ingress/DNS cutover from `frontend-prod` to the `production` release, not just a version bump.

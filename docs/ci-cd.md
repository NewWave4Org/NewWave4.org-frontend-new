...
2. **`development`'s required status checks** — this is required, not optional: `development`'s branch protection currently requires a check literally named `build`, produced by the old `docker_build.yml`. Reusable-workflow job checks are reported as `<caller job id>/<job name>` (e.g. `quality-gates / build`), not the bare job name — so even though `_quality-gates.yml`'s job is still called `build` for continuity, the check GitHub actually posts is `quality-gates / build`, which does **not** satisfy the old required-check name. An admin must update the required-checks list to `quality-gates / build` (and add `quality-gates / typecheck`, `quality-gates / unit-test`, `quality-gates / casing-guard`, `quality-gates / docker-smoke` as required too) — until this is done, PRs into `development` can stay pending forever waiting for a check name that will never be reported.

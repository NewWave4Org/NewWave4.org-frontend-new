...
## `next build` (Turbopack) crashes on a bare Ubuntu GitHub Actions runner

`_quality-gates.yml`'s `build` job originally ran `npm run build` directly on `ubuntu-latest` via `actions/setup-node`. This crashed deterministically and silently (no error message — Next.js printed "Skipping validation of types" then the process exited non-zero with nothing further) on every run, including after clearing GitHub's npm cache and confirming a fresh `npm ci`.

Investigation (2026-07-19): reproduced via a clean, `COPY`-based (not bind-mounted) Docker build on plain `node:20` — same crash. The same files, built the exact same way inside `node:20-alpine` (matching the Dockerfile's builder stage, i.e. what `docker-smoke` and the real deploy image actually use) — succeeded, repeatedly. Ruled out as causes: this repo's own code (no case-sensitivity import bugs — checked, every flagged path matches an on-disk file exactly), a poisoned npm cache (cleared and retried, same failure), and `--webpack` as an alternative (fails for unrelated pre-existing path-alias resolution reasons, not usable here).

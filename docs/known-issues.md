...
Resolution: removed the standalone `build` job. `docker-smoke` (via `docker/build-push-action`, a real `docker build` — never bind-mounted) already fully exercises `next build` through the exact Dockerfile used for real deploys, and has never failed across any of these runs — a more representative check than the standalone job was anyway. If this needs revisiting (e.g. to get a faster non-Docker build signal), be aware the crash reproduces on GitHub's infrastructure specifically and resists every local reproduction method tried so far.
